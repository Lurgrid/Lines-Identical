...
0,07s user 0,00s system 99`\%` cpu 0,061 total
    ...
    et celui de personne aillant réalisé un projet similaire. Afin de faire une
    bonne comparaison voir la figure~\ref{our-prog} pour comparer avec note 
    programme, pour ce test nous avons utiliser un fichier de 3Mo avec des 
    lignes de longeur au plus 80 charactères, nous avons donc un multiplicateur
    mémoire de 2,97.
...
...
0,13s user 0,00s system 98`\%` cpu 0,132 total
`\SU{user=root,host=ubuntu,color=lime}`
valgrind ./lnid_rol lesmiserables.txt `\StartConsole`
...
heap usage: 642,605 allocs, 642,605 frees, 21,728,522 bytes allocated
            \end{ubuntu}
            \captionsetup{position=bottom}
            \caption{.}
        \end{figure}

        Comme on peut voir ce projet à met deux fois plus de temps et il utilise
        plus d'espace mémoire, il a donc un multiplicateur mémoir de 7,23

        \vphantom{}
        
        \item[] \textbf{Tu-anh Le}
        \begin{figure}[H]
            \SU{user=root,host=ubuntu,color=lime}
            \begin{ubuntu}
./lnid_flo lesmiserables.txt `\StartConsole`
