...
\begin{document}
    ...
    \begin{ubuntu}
        ...
        `\SU{user=root,host=ubuntu,color=lime}`
        ./lnid princesse_de_cleves.txt `\StartConsole`
        princesse_de_cleves.txt
        ...
        0,00s user 0,00s system 92`\%` cpu 0,004 total
        `\SU{user=root,host=ubuntu,color=lime}`
        ./lnid princesse_de_cleves.txt --avl `\StartConsole`
        princesse_de_cleves.txt
        ...
    ...
    Comment vous pouvez voir dans ses différents textes de 1Mo à 2Mo avec des
    lignes de taille environs 80 caractères que les AVL sont légérement plus
    lent ou équivalant pour certain textes, les AVL et les table de hachage 
    utilise a peu près la espace mémoire. 

    On pourrait se poser la question de pourquoi avoir implémenter les AVL 
    alors, étant donné que les tables de hachages sont en théorie mieux, mais 
    quand théorie sur certain textes les AVL gagne en vitesse.


    %idée en théorie : avl < hastable
    % en pratique bonsoir non, sauf avl pour text avec ligne très long car y a 
    % pas à parcourir toute la chaine par une fonction de hashage. Par contre 
    % pour des plus petit fichier, la table de hashage est mieux.
    ...
    \begin{figure}[H]\label{test-cmp-f}
        \SU{user=root,host=ubuntu,color=lime}
        \begin{ubuntu}
