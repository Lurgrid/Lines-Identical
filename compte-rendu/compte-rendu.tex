...
\begin{document}
    ...
    alors le caractère sera d'abord transformé puis filtrer et enfin ajouté) 
    sur le fichier.
    ...
    \begin{enumerate}
        ...
        ligne à cette ligne un compteur initialement mis à 1 ou une liste de 
...
2       1       bugs
    ...
    notamment cité les table de hachage, dont il est courant pour chaque valeur 
    d'être stoker dans un maillon lui-même gérer par une liste allouée 
    dynamiquement (voir schéma d'implémentation du module hastable figure 
    \ref{hastable-fig}). De plus, certaine de ces implémentations de structure 
    peuvent engager des contraintes, telles que la suppression d'une de ces 
    allocations pour un certain traitement. On peut notamment citer des 
    implémentations d'ensemble à l'aide de liste. De telle structure n'aillant 
    pas de requête de parkour, il faut alors vidé toutes les valeurs de cet 
    ensemble pour les affichés par exemple. C'est dans ce cas que toutes les 
    allocations devraient être désallouées et alors, il  faudrait tous 
    ...
    lecture des lignes, le comptage des occurrences de celle-ci ou leurs 
    ...
    contiguës dans la mémoire et sans offset.
    ...
    il nous est impossible de la comparer aux chaines de caractère déjà lues 
    ...
    est déjà lue ou même dans le cas ou le fichier qui est lu n'est pas le 
    ...
    de hachage utilisé, c'est pour cela que dans la section de test, nous 
    ...
    hachage peut entraîner un agrandissement de celle-ci et donc, recalculé de 
    toutes les valeurs en sont sein. L'implémentation de cette table est 
    ...
    Divers tests aillant pour but d'identifier la meilleure option sont 
    ...
    par une chaine de caractère (représentation longue), mais aussi d'autre 
    chose présente dans les options Linux. 
    ...
    doit au moins exister. Toute option à possibilité d'interrompre le 
    ...
    le séparer d'un espace On peut faire appelle à plusieurs options courtes en 
    un seul  appelle, en suivant le '-` des caractères représentant les options 
    ...
    d'une et une seule option alors l'option préfixée sera appeler. Si elle est 
    préfixe de plusieurs options, alors elle sera considérer comme ambiguë et 
    conduira à la lever d'une erreur.
    ...
    pouvoir modifier certaine chose. Notamment les préfixe des options courtes 
    et longue (On peut notamment envisager des utilisateurs originaires de 
    ...
    valeur à traiter. Ce spécificateur vaux par défaut la chaine `- -'. De plus, 
    ...
    fonctionnalités des commandes Linux les plus connus (rm, ls, cat), notamment 
    des fonctionnalités telles que `l'autocomplétion' des longues options, la 
    prise en charge des multiples options courte ou encore le spécifier 
    ...
    représentent en aucun cas une promesse, étant donné que ces temps dépends de 
    nombreux facteurs. En revanche l'utilisation mémoire du programme pourra 
    être évoqué dans certaine comparaison. En effet, tous les tests étant 
    effectué sur la même machine, nous supposons donc que l'utilisation mémoire 
    du programme n'évolue pas pour un même traitement. De plus, les complexités 
    étant universelle, nous favoriserons l'étude de celle-ci.
    ...
    retrouverez ci-dessous une étude de divers tests traitent de ces cas.
...
0,01s user 0,01s system 98`\%` cpu 0,019 total
    ...
    théoriquement accessible en temps constant. Pour cela, nous devons revenir 
    sur la mise en pratique des tables de hachage. En effet, dans notre cas, 
    chaque ligne devra calculer par la fonction de hachage. Or cette fonction 
    (conseillé par Bernstein) effectue un traitement en temps linéaire 
    ...
    pire des cas de $O(nln*m)$ avec $n$ le nombre de lignes présente dans l'AVL 
    ...
    pour un tel exemple avec un fichier texte générer à l'aide du programme de 
    la figure \ref{prog}. 
...
9,46s user 0,71s system 99`\%` cpu 10,169 total
    ...
    cas où l'utilisateur connait la taille moyenne de ces lignes, il lui 
    pourrait être utile d'utiliser alors l'AVL\@.
    ...
    catégories (avl exclu aillant déjà était traité longuement dans la partie 
    \ref{avl-has}):
    ...
    \begin{enumerate}
        ...
        sont pas constante, mais qu'une certaine différence semble apparaitre.
...
heap usage: 35,036 allocs, 35,036 frees, 4,002,711,191 bytes allocated
    ...
    ici deux. Le premier à une taille de 1,3 Ko (d'une dizaine de ligne) et le 
    second de 64 Go (de 5000 lignes). Il n'existe aucune ligne similaire entre 
    ces deux fichiers. Voir figure~\ref{fig:4vs60} pour ce test.
...
heap usage: 254 allocs, 254 frees, 33,573,081 bytes allocated
    ...
    On peut calculé à partir de notre implémentation l'espace utiliser pour le
    buffeur en total, la taille des lignes étant fixée a 12 799 999 charactèes,
    on peut en conclure que la taille de celui ci sera égal 
    $2^{\left\lceil \log_2 12 799 999\right\rceil} = 2^{24} = \numprint{16777216}$ 
    charactères. La machine executant le test représente les caractères sur un 
    byte, on en conclue que le buffeur à sa taille final répresente plus de la 
    moitier de la mémoire utiliser. Tout au long de l'éxecution a donc utiliser:
    ...
    \begin{figure}[H]
        ...
        \begin{equation}
            ...
            \end{split}
        \end{equation}
        \caption{Calcule du total de l'utilisation mémoire du buffeur}
    \end{figure}

    Pour conclure, on a donc que 99,94\% de l'utilisation mémoire qui risulte du
    buffer, il nous donc difficilement possible d'améliorer ces résultats.
